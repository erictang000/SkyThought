...
class Response:
    ...
    def from_ray_response(cls, response) -> 'Response':
        ...
        if isinstance(response["generated_text"], list):
            # n > 1 samples
            num_completion_tokens = [int(response["num_generated_tokens"][i]) for i in range(len(response["num_generated_tokens"]))] 
        else:
    ...
    def from_vllm_response(cls, response) -> 'Response':
        ...
        """
        response_text = [response.outputs[i].text for i in range(len(response.outputs))] if len(response.outputs) > 1 else response.outputs[0].text
        num_completion_tokens = [len(s) for s in response_text] if not isinstance(response_text, str) else len(response_text)
        return cls(
